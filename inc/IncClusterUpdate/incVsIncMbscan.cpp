...
void writeMatrixFileInt(vector<vector<int > > matrix, string fname);
void writeMatrixFileDouble(vector<vector<double > > matrix, string fname);

vector<vector<int > > readMatrixFileInt(string fname);
vector<vector<double > > readMatrixFileDouble(string fname);
...
int main(int argc, char* argv[]){
    ...
    writeMatrixFileInt(splitAttribute,"static/splitAttribute");
    writeMatrixFileDouble(splitPoint,"static/splitPoint");
    writeMatrixFileInt(NodeSize,"static/NodeSize");
    writeMatrixFileInt(treeNode,"static/treeNode");
    writeMatrixFileInt(treeNodeMass,"static/treeNodeMass");
    writeMatrixFileDouble(massMatrix,"static/massMatrix");
 ...
 for(int loop = 0;loop < 10 ;loop++){
    ...
    splitAttribute = readMatrixFileInt("static/splitAttribute");
    splitPoint = readMatrixFileDouble("static/splitPoint");
    NodeSize = readMatrixFileInt("static/NodeSize");
    treeNode = readMatrixFileInt("static/treeNode");
    treeNodeMass = readMatrixFileInt("static/treeNodeMass");
    massMatrix = readMatrixFileDouble("static/massMatrix");
    ...
    //cout<<"total running time inc: "<<(double)(clock() - tStart_iForest_inc)/CLOCKS_PER_SEC<<" sec"<<endl;
        ...
        //cout << "inc  mbs done" << endl;
   
    writeCluster(nInst+insts, loop);
...
void incMuNeighborUpdate(int insts, int loop){
  ...
  stringstream ss;
  ss << loop;
  string massOutFile = "massCluster/massIncInc"+ss.str();
...
void writeCluster(int n, int loop){
    ...
    stringstream ss;
    ss << loop;
    string compareFile = "massCluster/clusterIncInc"+ss.str();
...
void writeMatrixFileInt(vector<vector<int > > matrix, string fname){
  ofstream matFile(fname.c_str(),ios::out);
  matFile<<""; 
  matFile.close();
  //Write the root node to a file
  matFile.open(fname.c_str(),ios::app);
...
vector<vector<int > > readMatrixFileInt(string fname){
  ...
  if(!in)
  {
      cout<<"Cannot open the input file: "<<fname<<endl;
      exit(0);
  } 
  int rows,cols;
  in >> rows;
